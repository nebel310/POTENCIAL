...
def initial(s: string):
    """
    Создает инициалы на основе имени.

    Args:
        s (string): Имя студента.

    Returns:
        string: Инициалы, сформированные на основе первой буквы имени, фамилии и отчества.
    """
    print(s)
    names = s.split()
    print(names)
...
def create_password():
    ...
    Returns:
        string: Сгенерированный случайный пароль.
    """
    characters = string.ascii_letters + string.digits
    print(characters)
    password = ''.join(random.choice(characters) for _ in range(8))
